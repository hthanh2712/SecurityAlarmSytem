\hypertarget{group__xTaskNotifyIndexedFromISR}{}\doxysection{x\+Task\+Notify\+Indexed\+From\+ISR}
\label{group__xTaskNotifyIndexedFromISR}\index{xTaskNotifyIndexedFromISR@{xTaskNotifyIndexedFromISR}}
task. h 
\begin{DoxyPre}BaseType\_t xTaskNotifyIndexedFromISR( TaskHandle\_t xTaskToNotify, UBaseType\_t uxIndexToNotify, uint32\_t ulValue, eNotifyAction eAction, BaseType\_t *pxHigherPriorityTaskWoken );\end{DoxyPre}
...
A version of x\+Task\+Notify\+Indexed() that can be used from an interrupt service routine (ISR).
...
Backward compatibility information\+: Prior to Free\+RTOS V10.\+4.\+0 each task had a single \char`\"{}notification value\char`\"{}, and all task notification API functions operated on that value. Replacing the single notification value with an array of notification values necessitated a new set of API functions that could address specific notifications within the array. x\+Task\+Notify\+From\+ISR() is the original API function, and remains backward compatible by always operating on the notification value at index 0 within the array. Calling x\+Task\+Notify\+From\+ISR() is equivalent to calling x\+Task\+Notify\+Indexed\+From\+ISR() with the ux\+Index\+To\+Notify parameter set to 0.


\begin{DoxyParams}{Parameters}
{\em ux\+Index\+To\+Notify} & The index within the target task\textquotesingle{}s array of notification values to which the notification is to be sent. ux\+Index\+To\+Notify must be less than config\+TASK\+\_\+\+NOTIFICATION\+\_\+\+ARRAY\+\_\+\+ENTRIES. x\+Task\+Notify\+From\+ISR() does not have this parameter and always sends notifications to index 0.\\
\hline
